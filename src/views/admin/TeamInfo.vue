...
<template>
  <div class="w-full h-full flex flex-col p-2 md:p-3 gap-4">
    <!-- Header de l'équipe -->
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <section
        class="col-span-2 p-4 flex justify-between items-center bg-surface rounded-lg border-2 border-border shadow-md"
      >
      ...
      <section
        class="col-span-2 p-4 grid grid-cols-2 gap-4 border-2 border-border rounded-lg bg-surface"
      >
    ...
    <div
      class="flex flex-wrap gap-2 justify-between sm:flex-row sm:space-y-0 space-y-6 md:space-y-0"
    >
      <!-- Statistiques -->
      <section
        class="w-full sm:w-[49%] p-4 bg-surface border-2 border-border rounded-lg overflow-y-auto shadow-md"
        style="max-height: 400px"
      >
        <h3 class="font-bold text-xl">
          Statistiques
          <span class="hidden sm:inline">
            de {{ stores.getOperatorById(activeOperatorId)?.fname }}
