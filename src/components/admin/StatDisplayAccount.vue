...
<template>
  <div v-if="stats" class="space-y-3 grid md:grid-cols-2 lg:grid-cols-3">
    <div v-for="(statList, date) in stats" :key="date">
      <h2 class="text-xl font-semibold text-muted mb-1">
        {{ new Date(date).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long' }) }}
      </h2>

      <div :class="statList.length === 1 ? 'lg:flex gap-2 mb-1' : 'grid grid-cols-1 gap-2 mb-1'">
        <div
        ...
        >
          ...
          <div class="flex gap-2">
            <p class="flex items-center gap-1 text-text">
              ...
              Entrant: <span class="font-semibold">{{ item.entry_total }}</span>
            </p>
            <p class="flex items-center gap-1 text-text">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                <g
              ...
              Stop: <span class="font-semibold">{{ item.stop_total }}</span>
            </p>
          </div>
        </div>
      </div>
  ...
  <div v-else class="text-muted text-center py-4">Aucune statistique disponible.</div>
  ...
  >
    <div class="bg-surface rounded-lg p-4 border border-border max-w-md w-full shadow-lg">
      ...
      <div class="grid grid-cols-3 gap-1.5 text-text">
        ...
        <p>
          <span class="font-semibold">Début :</span> {{ selectedStat?.entry_start }}
          <sup>{{ selectedStat?.stop_start }}</sup>
        </p>
        <p>
          <span class="font-semibold">Fin :</span> {{ selectedStat?.entry_end }}
          <sup>{{ selectedStat?.stop_end }}</sup>
        </p>
        <p>
          <span class="font-semibold">Somme :</span> {{ selectedStat?.entry_total }}
